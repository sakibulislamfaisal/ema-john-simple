...
const Shipment = () => {
    ...
    const onSubmit = data => {
          ...
          const orderDetail = {email:auth.user.email , cart: saveCart , name:auth.user.name};
          //console.log(data)
          fetch('http://localhost:4200/orderPlaced',{
            method : 'POST',
            ...
            headers: {
                 ...
                 .then(data =>{
                   console.log(data);
                   alert('Order Placed Successfully by Order Id is  : ' + data._id);
                   processOrder();
